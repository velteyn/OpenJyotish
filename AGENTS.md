...
| **Jhora** | `the development repository` | Private | Development, research reference |
...
git add src/ tests/
# NEVER commit: data/jhora.db (test churn), RE artifacts (openjyotish.exe, *.dll, *.hlp, *.cnt)
...
| What | When |
|------|------|
| `the gap analysis.md` | New calculation added or gap closed |
...
- `openjyotish.exe`, `swedll32.dll`, `jhora.hlp` — original the tradition
...
- `docs/help/` — original the tradition help files
...
1. **Never push to public before testing** — development repository has ephemeris data for real testing.
2. **Always clear `__pycache__`** when debugging stale behavior — Python caches old `.pyc`
   files that persist across git pulls.
3. **Always commit from the development repository** — the public repo is a mirror, not a workspace.
