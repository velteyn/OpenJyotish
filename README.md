# the classical tradition — Vedic Astrology Software
...
### File menu
- **Import .jhd** — Load original the tradition format charts
- **Save to Database** — Stores chart in SQLite for later browsing
- **Export .jhd** — Save in original the tradition format
...
- Python 3.11+
- Swiss Ephemeris data files (`.se1` files) in `jhcore/ephe/` — these are the original the tradition ephemeris files (not distributed in git)

```bash
# If you have the original the tradition installation, copy the ephemeris files:
...
- **JHD import/export** — compatible with original the tradition `.jhd` format
