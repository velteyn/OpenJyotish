...
def chart_to_json(cd: ChartData, usl_config=None) -> Dict[str, Any]:
    ...
    from jhora.calc.drishti import houses_aspected as _houses_asp
    _lsi = int(cd.ascendant // 30) % 12
    _asp = {g: _houses_asp(g, int(p.longitude // 30) % 12, _lsi)
    ...
    try:
        ...
        for sys in ("ashtottari", "yogini", "sudasa", "chara",
                    "narayana", "lagnamsaka", "padanaathaamsa", "rasi-bhukta-vimsottari", "kalachakra", "brahma", "karaka",
                    "shoola", "trikona", "varnada",
                    "sthira", "navamsa", "yogardha", "niryana-shoola",
                    "lagna-kendradi", "kaala", "chakra", "mandooka", "buddhi-gati", "drig",
                    "sudarshana",
    ...
    try:
        ...
        result["kp"] = {
            ...
            "dasa_lords": [
                {
                    "lord": r.graha.full_name,
                    "start_jd": round(r.start_jd, 4),
                    "end_jd": round(r.end_jd, 4),
                    "duration_years": round(r.duration_years, 4),
                    "house": r.house,
                    "sign_lord": r.chain.sign_lord.full_name,
                    "star_lord": r.chain.star_lord.full_name,
                    "sub_lord": r.chain.sub_lord.full_name,
