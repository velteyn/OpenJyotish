...
_SHORT_PLANET = {
    ...
    Graha.MERCURY: "Merc", Graha.JUPITER: "Jupt",
    Graha.VENUS: "Venu", Graha.SATURN: "Satn",
    Graha.RAHU: "Rahu", Graha.KETU: "Ketu",
}

_OUTER_ABBR = {"Uranus": "Uran", "Neptune": "Nept", "Pluto": "Plut"}
...
_OUTER_TABLE = {"Uranus": "Uran", "Neptune": "Nept", "Pluto": "Plut"}
...
def _planet_rows(cd: ChartData) -> List[List[str]]:
    ...
    for g in [Graha.SUN, Graha.MOON, Graha.MARS, Graha.MERCURY,
        ...
        p = cd.planet(g)
        _row(_SHORT_PLANET[g], p.longitude, p.is_retrograde)
    for oname, abbr in _OUTER_TABLE.items():
        od = cd.outer_planets.get(oname)
        if od:
            _row(abbr, od["longitude"], od.get("is_retrograde", False))
    return rows
...
def _ashtakavarga_rows(cd: ChartData) -> Tuple[List[str], List[List[str]]]:
    ...
    rows = []
    for g in _AV_SUBJECTS:
        rows.append([_SHORT_PLANET[g]] + [str(v) for v in bavs[g]])
...
def render_traditional_report(cd: ChartData, name: str = "", sex: str = "",
    ...
    # ── Layout constants — A4 at 180 DPI ──
    W = 1488
    M = 45                  # page margin (15px padding from border)
    CW = W - 2 * M            # 1398
    GAP = 14
    SGAP = 10
    ...
    # Left 2/3: Lagna Chart + Vimshottari Dasha
    left_w = int(CW * 0.63)   # ~880
    # Right 1/3: Navamsa + Planetary Positions + Ashtakvarga + Chalit
    ...
    box_row_h = 29
    box_h = 48 + 9 * box_row_h   # 309 per box
    dasha_title_h = 34
    dasha_grid_h = 3 * box_h + 2 * SGAP   # 927

    # Right-side tables
    table_row_h = 27
    planets_h = 28 + 24 + 13 * table_row_h   # 403
    av_h = 28 + 24 + 8 * table_row_h          # 268
    chalit_h = 28 + 24 + 12 * table_row_h     # 376
    footer_h = 40

    # Page height: use the taller column
    ...
    # ── Header box — labels left, data right-justified per column ──
    painter.setPen(QPen(ink, 1.8))
    painter.drawRoundedRect(QRectF(M, y, CW, header_h), 10, 10)
    cols = _header_columns(cd, name, sex, place)
    # 4 equal columns within content width, with inner padding
    pad = 15
    col_gap = 20
    col_w = (CW - 2 * pad - 3 * col_gap) / 4
    col_x = [M + pad + i * (col_w + col_gap) for i in range(4)]
    label_font = QFont("Arial", 13, QFont.Weight.Bold)
    value_font = QFont("Arial", 13)
    row_h = (header_h - 2 * pad) / 6.0
    for ci, col in enumerate(cols):
        ...
        for label, value in col:
            ...
            # Right-justify value within its column
            fm = painter.fontMetrics()
            vw = fm.horizontalAdvance(value)
            painter.drawText(int(col_x[ci] + col_w - vw), int(ry + 14), value)
            ry += row_h
    y += header_h + GAP

    # ── Charts: Lagna (left 2/3) | Navamsa (right 1/3) ──
    ...
    for bi, box in enumerate(boxes):
        ...
        painter.drawText(QRectF(bx, by + 3, box_w, 20),
                         Qt.AlignmentFlag.AlignCenter,
                         f"{lord3} - {box['full_years']} Years")
        painter.drawText(QRectF(bx, by + 22, box_w, 20),
                         Qt.AlignmentFlag.AlignCenter,
                         f"{box['start']} - {box['end']}")
        painter.setPen(QPen(ink, 1.0))
        painter.drawLine(int(bx), int(by + 44), int(bx + box_w), int(by + 44))
    ...
    painter.drawText(QRectF(0, page_h - footer_h, W, 20),
                     Qt.AlignmentFlag.AlignCenter,
                     f"Generated by OpenJyotish — {now}")
