...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        self.setWindowTitle("the classical tradition — Vedic Astrology")
    ...
    def _on_file_open(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Open Chart Data", "", "Chart Data (*.jhd);;All Files (*)"
        )
        if not path:
            return
        try:
            ...
            self.setWindowTitle(f"the classical tradition — {data.name}")
    ...
    def _on_file_save_as(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Chart Data", "", "Chart Data (*.jhd);;All Files (*)"
        )
        if path:
    ...
    def _init_atlas(self) -> Optional[AtlasReader | StaticAtlasReader]:
        if self._atlas is not None:
            return self._atlas
        try:
            self._atlas = open_default_atlas(Path(__file__).resolve().parents[3])
